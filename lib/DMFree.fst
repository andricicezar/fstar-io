...
val theta : #a:Type u#a -> #cmd:(Type u#0 -> Type u#e) -> #event:Type -> cwp:cmd_wp cmd event -> m:ffree cmd a -> hist #event a
let rec theta #a #cmd #event cwp m =
  match m with
  | Return x -> hist_return x
  | Call op k ->
      hist_bind (cwp op) (fun ri -> theta cwp (k ri))

let lemma_theta_is_monad_morphism_ret #cmd (#event:Type) (cwp:cmd_wp cmd event) (v:'a) :
  Lemma (theta cwp (ffree_return #cmd v) == hist_return v) by (compute ()) = ()
...
let rec lemma_theta_is_lax_morphism_bind #a #b #cmd (#event:Type) (cwp:cmd_wp cmd event) (m:ffree cmd a) (f:a -> ffree cmd b) :
  Lemma
    (theta cwp (ffree_bind m f) ⊑ hist_bind (theta cwp m) (fun x -> theta cwp (f x))) =
  match m with
  | Return x ->
    calc (hist_ord) {
      ...
      theta cwp (f x);
      == {} // unfold ffree_bind
      theta cwp (ffree_bind (Return x) f);
      == {}
      theta cwp (ffree_bind m f);
    }
  | Call op k ->
    calc (hist_ord) {
      hist_bind (theta cwp m) (fun x -> theta cwp (f x));
      == {
        assert (hist_bind (theta cwp (Call op k)) (fun x -> theta cwp (f x))
           == hist_bind (theta cwp m) (fun x -> theta cwp (f x))) by (rewrite_eqs_from_context ())
      }
      hist_bind (theta cwp (Call op k)) (fun x -> theta cwp (f x));
      == { _ by (compute ()) } // unfold theta
      hist_bind (hist_bind (cwp op) (fun ri -> theta cwp (k ri))) (fun x -> theta cwp (f x));
      `hist_equiv` { lemma_hist_bind_associativity (cwp op) (fun ri -> theta cwp (k ri)) (fun x -> theta cwp (f x)) }
      hist_bind (cwp op) (fun ri -> hist_bind (theta cwp (k ri)) (fun x -> theta cwp (f x)));
      `hist_ord` {
        let rhs1 = fun ri -> hist_bind (theta cwp (k ri)) (fun x -> theta cwp (f x)) in
        let rhs2 = fun ri -> theta cwp (ffree_bind (k ri) f) in
        introduce forall ri. (rhs1 ri) `hist_ord` (rhs2 ri) with begin
          lemma_theta_is_lax_morphism_bind cwp (k ri) f
        end;
        another_lemma' (cwp op) rhs1 rhs2;
        assert (hist_bind (cwp op) rhs1 `hist_ord` hist_bind (cwp op) rhs2) by (assumption ())
      }
      hist_bind (cwp op) (fun ri -> theta cwp (ffree_bind (k ri) f));
      == { _ by (compute ()) } // unfold theta
      theta cwp (Call op (fun ri -> ffree_bind (k ri) f));
      `hist_ord` { _ by (compute ()) } // unfold ffree_bind
      theta cwp (ffree_bind (Call op k) f);
      == {}
      theta cwp (ffree_bind m f);
    }
...
type dm (cmd:Type0 -> Type) (event:Type) (cwp:cmd_wp cmd event) (a:Type) (wp:hist #event a) =
  (m:(ffree cmd a){theta cwp m ⊑ wp})

let dm_return #cmd (#event:Type) (cwp:cmd_wp cmd event) #a (x : a) : dm cmd event cwp a (hist_return #a #event x) =
  ffree_return x

let dm_cmd #cmd (#event:Type) (cwp:cmd_wp cmd event) #r (op:cmd r) :
  dm cmd event cwp r (hist_bind (cwp op) (fun ri -> hist_return ri)) =
  Call op Return
