...
val theta : #a:Type u#a -> #cmd:(Type u#0 -> Type u#e) -> #event:Type -> cwp:cmd_wp cmd event -> m:free cmd a -> hist #event a
...
let rec lemma_theta_is_lax_morphism_bind #a #b #cmd (#event:Type) (cwp:cmd_wp cmd event) (m:free cmd a) (f:a -> free cmd b) :
  ...
  | Call op k ->
    calc (hist_ord) {
      ...
      hist_bind (cwp op) (fun ri -> theta cwp (free_bind (k ri) f));
      == { _ by (compute ()) } // unfold theta
      theta cwp (Call op (fun ri -> free_bind (k ri) f));
      `hist_ord` { _ by (compute ()) } // unfold free_bind
      theta cwp (free_bind (Call op k) f);
      == {}
      theta cwp (free_bind m f);
    }
