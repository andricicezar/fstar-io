...
let parse_http_header (header : Unsafe.Bytes.bytes) : request_type =
  match Unsafe.Bytes.iutf8_opt header with
  | None -> Error
  | Some header ->
...
let rec get_file
  ...
  (i:nat) :
  MIOpi Unsafe.Bytes.bytes fl sgm mymst =
  match call_io Read (fd,limit) with
  | Inl (chunk, size) -> begin
    if UInt8.lt size limit || i = 0 then
      Unsafe.Bytes.sub chunk 0ul (UInt32.uint_to_t (UInt8.v size))
    else (
      Unsafe.Bytes.append chunk (get_file call_io fd limit (i-1))
    )
  end
  | _ -> Unsafe.Bytes.utf8_encode ""

(**let write_string
  (#fl:erased tflag)
  (send:Unsafe.Bytes.bytes -> MIOpi (resexn unit) fl pi)
  (s:string) : MIOpi unit fl pi =
    let _ = send (Unsafe.Bytes.utf8_encode s) in ()
...
let respond
  (#fl:erased tflag)
  (send:Unsafe.Bytes.bytes -> MIOpi (resexn unit) fl sgm mymst)
  (status_code:int) (media_type:string) (content:Unsafe.Bytes.bytes) :
  MIOpi unit fl sgm mymst =
  let hdrs = set_headers status_code media_type (Unsafe.Bytes.length content) in
  let msg = (Unsafe.Bytes.append (Unsafe.Bytes.utf8_encode hdrs) content) in
  let _ = send msg in
  ()
...
let get_query
  ...
  match get_fd_stats call_io file_full_path with | Inr _ -> () | Inl (fd, stat) -> begin
    ...
    let msg = (Unsafe.Bytes.append (Unsafe.Bytes.utf8_encode hdrs) file) in
    let _ = send msg in
