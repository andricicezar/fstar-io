...
let rec construct_steps_ecall
  ...
  Lemma
    (requires indexed_irred e' (h++lt))
    (ensures steps (ECall op e) (ECall op e') h lt)
    (decreases st) =
  match st with
  | SRefl _ _ -> ()
  | STrans #_ #e_ #_ #_ #oev #lt23 step_e rest -> begin
    let _ : step (ECall op e) (ECall op e_) h oev = SCall step_e in
    lem_step_implies_steps (ECall op e) (ECall op e_) h oev;
    let lt' : local_trace h = as_lt oev in
    trans_history h lt' lt23;
    construct_steps_ecall op e_ e' (h++lt') lt23 rest;
    lem_steps_transitive (ECall op e) (ECall op e_) (ECall op e') h lt' lt23
    end
