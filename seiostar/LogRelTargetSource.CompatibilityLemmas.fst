...
(** Auxiliary: a value-level relation transports across a retyping whenever
    ...
    [Prims.has_type]: "F*'s type theory certainly does not internalize its
    own typing judgment"). Without this derivation, the recursive call that
    transports an arrow input from [bt1 ∋] to [at1 ∋] cannot be made in
    standard F*. *)
#push-options "--z3rlimit 60 --fuel 2 --ifuel 2"
let rec lem_retype_value (a b : qType) (h : history) (v : fs_val a) (e : closed_exp)
  : Lemma
  ...
  | QPair #bt1 #bt2 qtb1 qtb2 -> begin
    ...
    | QPair #at1 #at2 qta1 qta2 ->
      let EPair e1 e2 = e in
      lem_retype_value (QTypes.pack qta1) (QTypes.pack qtb1) h (fst #at1 #at2 v) e1;
      lem_retype_value (QTypes.pack qta2) (QTypes.pack qtb2) h (snd #at1 #at2 v) e2
    | _ -> ()
    end
  | QSum #bt1 #bt2 qtb1 qtb2 -> begin
    ...
    | QSum #at1 #at2 qta1 qta2 -> begin
      ...
      match v_a, e with
      | Inl v', EInl e' ->
        lem_retype_value (QTypes.pack qta1) (QTypes.pack qtb1) h v' e'
      | Inr v', EInr e' ->
        lem_retype_value (QTypes.pack qta2) (QTypes.pack qtb2) h v' e'
      | _ -> ()
      end
    | _ -> ()
    end
...
(** Pointwise lift to [⊇]. *)
let lem_retype_pointwise_superset
  ...
  introduce forall (e':closed_exp) (lt:local_trace h).
    e_beh e e' h lt ==> (b ∋ (h, v, e') /\ lt == [])
  with begin
    introduce e_beh e e' h lt ==> (b ∋ (h, v, e') /\ lt == []) with _. begin
      assert (a ∋ (h, v, e'));
      lem_retype_value a b h v e'
    end
  end
...
let compat_oval_retype #g (#a:qType) (#preV:spec_env g)
  ...
  assert_norm (superset_oval b (fs_oval_retype fs_v b) e ==
    (fv_in_env g e /\
     (forall bo (s:gsub g bo) (fsG:eval_env g) (h:history).
  ...
  with begin
    introduce (fsG `(∽) h` s /\ (spec_env_retype b fs_v) fsG) ==>
      ...
      lem_retype_pointwise_superset a b h (fs_v fsG) (gsubst s e);
