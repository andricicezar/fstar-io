// ER: This module is a sub-copy from rsp.fst,
//   adding some definitions related to the
//   monadic nature of set_of_traces/behavior
...
let set_of_traces_map f p (es, b) = exists a . f a == b /\ p (es, a)
  (* set_of_traces_map f p = set_of_traces_bind p (fun a => set_of_traces_ret (f a))
                           = set_of_traces_bind p (fun a => fun (es, x') => f a == x' /\ es == [])
                           = fun (es', b') => exists (es1 es2 : events_trace) . exists a . es == (es1 @ es2) /\ p (es1, a) /\ f a == b /\ es2 == []
                           = fun (es', b') => exists (es1 : events_trace) . exists a . es == es1 /\ p (es1, a) /\ f a == b
                           = fun (es', b') => exists a . p (es, a) /\ f a == b  *)

val included_in_same : (#a:Type) -> set_of_traces a -> set_of_traces a -> Type0
let included_in_same s1 s2 = forall t r. s1 (t, r) ==>  s2 (t, r)

val included_in' : (#a:Type) -> (#b:Type) -> (b -> a) -> set_of_traces a -> set_of_traces b -> Type0
let included_in' rel s1 s2 = included_in_same s1 (set_of_traces_map rel s2)

val equiv : (#a : Type) -> set_of_traces a -> set_of_traces a -> Type0
let equiv s1 s2 = included_in_same s1 s2 /\ included_in_same s2 s1
...
val set_of_traces_ret' : (#a:Type) -> a -> set_of_traces' a
let set_of_traces_ret' x (es, x') = Inl x == x' /\ es == []

val set_of_traces_bind' : (#a:Type) -> (#b:Type) -> set_of_traces' a -> (a -> set_of_traces' b) -> set_of_traces' b
let set_of_traces_bind' p f (es, b) = exists es1 es2 . exists a . es == (es1 @ es2) /\ p (es1, a) /\ (Inr? a ==> es2 == [] /\ b == Inr (Inr?.v a)) /\ (Inl? a ==> f (Inl?.v a) (es2, b))
  (* set_of_traces_bind' p f = set_of_traces_bind p (fun a -> match a with { Inr e -> set_of_traces_ret (Inr e); Inl a -> f a })
                             = fun (es', b') -> exists es1 es2 . exists a . es' == (es1 @ es2) /\ p (es1, a) /\ match a with { Inr e -> es2 == [] /\ b == Inr e; Inl a -> f a (es2, b) }
                             = fun (es', b') -> exists es1 es2 . exists a . es' == (es1 @ es2) /\ p (es1, a) /\ (Inr? a ==> es2 == [] /\ b == Inr e) /\ (Inl? a ==> f (Inl?.v a) (es2, b))
  *)
...
let behavior' #a
  (m : io a) : set_of_traces' a = behavior m
...
let beh_extend_trace_in_bind 
  ...
  calc (==) {
    ...
    == { admit () } //; _ by (unfold_def(`sysf_fmap); norm [iota]; unfold_def(`io_bind))
...
let beh_bind'
  ...
  (m : io a)
  (k : a -> io b) :
  Lemma (set_of_traces_bind' (behavior m) (fun a -> behavior (k a)) `included_in id` behavior (io_bind _ _ m k)) by (unfold_def (`included_in); unfold_def (`set_of_traces_bind'))
  = beh_bind m k
