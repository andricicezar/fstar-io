...
unfold
let value #a (m : iotree a) =
  x : a { x `return_of` m }

let bcast #a p q (b : branch (x : a { p x })) :
  Pure (branch (x : a { q x })) (requires forall x. p x ==> q x) (ensures fun _ -> True) =
  match b with
  | Fin tr res -> Fin tr res
  | Inf p -> Inf p

let wcast #a p q (w : wp (x : a { p x })) :
  Pure (wp (x : a { q x })) (requires forall x. p x ==> q x) (ensures fun _ -> True) =
  fun post hist -> w (fun b -> post (bcast p q b)) hist

unfold
let wforget #a (#m : iotree a) (w : wp (value m)) : wp a =
  wcast (fun x -> x `return_of` m) (fun x -> True) w

(** Effect observation *)
let theta #a (t : iotree a) : wp (value t) =
  fun (post : wpost (value t)) (hist : history) ->
...
let iodiv a (w : wp a) =
  t: iotree a { wforget (theta t) `wle` w }
...
let winv_fin (pre : history -> Type0) (inv : trace -> Type0) post hist r :
  Lemma
    (requires winv pre inv post hist /\ terminates r /\ pre (hist_cons hist (ret_trace r)) /\ inv (ret_trace r))
    (ensures post r)
= ()

let winv_inf (pre : history -> Type0) (inv : trace -> Type0) post hist r :
  Lemma
    (requires winv pre inv post hist /\ diverges r /\ periodically (fun n -> inv (inf_prefix r n)))
    (ensures post r)
= ()

let winv_ret (pre : history -> Type0) (inv : trace -> Type0) (t : iodiv unit (winv pre inv)) (hist : history) (p : iopos) :
  Lemma
    (requires pre hist /\ isRet (t p))
    (ensures pre (hist_cons hist (ipos_trace p)) /\ inv (ipos_trace p) /\ valid_trace hist (ipos_trace p))
= wprepost_id_inst pre (fun hist r ->
    (terminates r ==> pre (hist_cons hist (ret_trace r)) /\ inv (ret_trace r)) /\
    (diverges r ==> periodically (fun n -> inv (inf_prefix r n)))
  ) t hist

let winv_event_stream (pre : history -> Type0) (inv : trace -> Type0) (t : iodiv unit (winv pre inv)) (hist : history) (p p' : iopostream) :
  Lemma
    (requires pre hist /\ event_stream t p /\ p `uptotau` p')
    (ensures periodically (fun n -> inv (ipos_trace (stream_trunc p' n))) /\ valid_postream hist p')
= wprepost_id_inst pre (fun hist r ->
    (terminates r ==> pre (hist_cons hist (ret_trace r)) /\ inv (ret_trace r)) /\
    (diverges r ==> periodically (fun n -> inv (inf_prefix r n)))
  ) t hist ;
  theta_event_stream (winv pre inv) t (fun r ->
    (terminates r ==> pre (hist_cons hist (ret_trace r)) /\ inv (ret_trace r)) /\
    (diverges r ==> periodically (fun n -> inv (inf_prefix r n)))
  ) hist p p' ;
  assert (diverges (Inf #unit p'))
...
let append_stable_inst (inv : trace -> Type0) (tr tr' : trace) :
  Lemma
    (requires append_stable inv /\ inv tr /\ inv tr')
    (ensures inv (tr @ tr'))
= ()
...
let uptotau_trace_implies_periodically (pr : trace -> Type0) (p p' : iopostream) :
  Lemma
    (requires p `uptotau` p' /\ periodically (fun n -> pr (ipos_trace (stream_trunc p n))))
...
let repeat_proof_aux_smaller (body : iotree unit) (n : nat) (p : iopostream) q q' :
  Lemma
...
let repeat_proof_aux_eqpos (p : iopostream) (p0 q' : iopos) (n : nat) :
  Lemma
    (requires
      n >= 1 + length p0 /\
...
let repeat_proof_aux_eqtrace (p : iopostream) (p0 q' : iopos) (n m : nat) :
  Lemma
    ...
    (ensures
      ipos_trace (stream_trunc p (m + 1 + length p0)) ==
      ipos_trace p0 @ ipos_trace (stream_trunc (stream_drop (1 + length p0) p) m)
    )
= calc (==) {
    stream_trunc (stream_drop (1 + length p0) p) m ;
    == {}
    stream_trunc (stream_drop (1 + length p0) p) ((n - 1 - length p0) + (m - (n - 1 - length p0))) ;
    == { stream_trunc_add (stream_drop (1 + length p0) p) (n - 1 - length p0) (m - (n - 1 - length p0)) }
    stream_trunc (stream_drop (1 + length p0) p) (n - 1 - length p0) @ stream_trunc (stream_drop (n - 1 - length p0) (stream_drop (1 + length p0) p)) (m - (n - 1 - length p0)) ;
    == { stream_drop_drop (n - 1 - length p0) (1 + length p0) p ; forall_intro_2 iopostream_ext }
    stream_trunc (stream_drop (1 + length p0) p) (n - 1 - length p0) @ stream_trunc (stream_drop ((1 + length p0) + (n - 1 - length p0)) p) (m - (n - 1 - length p0)) ;
    == {}
    stream_trunc (stream_drop (1 + length p0) p) (n - 1 - length p0) @ stream_trunc (stream_drop n p) (m - (n - 1 - length p0)) ;
    == { repeat_proof_aux_eqpos p p0 q' n }
    q' @ stream_trunc (stream_drop n p) (m - (n - 1 - length p0)) ;
  } ;
  calc (==) {
    ipos_trace p0 @ ipos_trace (stream_trunc (stream_drop (1 + length p0) p) m) ;
    == {}
    ipos_trace p0 @ ipos_trace (q' @ stream_trunc (stream_drop n p) (m - (n - 1 - length p0))) ;
    == {}
    ipos_trace p0 @ ipos_trace (Tau_choice :: (q' @ stream_trunc (stream_drop n p) (m - (n - 1 - length p0)))) ;
    == { forall_intro_2 ipos_trace_append }
    ipos_trace (p0 @ Tau_choice :: q' @ stream_trunc (stream_drop n p) (m - (n - 1 - length p0))) ;
    == { forall_intro_2 (append_assoc p0) }
    ipos_trace ((p0 @ Tau_choice :: q') @ stream_trunc (stream_drop n p) (m - (n - 1 - length p0))) ;
    == {}
    ipos_trace (stream_trunc p n @ stream_trunc (stream_drop n p) (m - (n - 1 - length p0))) ;
    == { stream_trunc_add p n (m - (n - 1 - length p0)) }
    ipos_trace (stream_trunc p (n + (m - (n - 1 - length p0)))) ;
  }

let iodiv_repeat_proof_aux_inf (pre : history -> Type0) (inv : trace -> Type0)
  (body : iodiv unit (winv pre inv)) (post : wpost unit) (hist : trace) (p : iopostream) (n : nat) :
  Lemma
    (requires append_stable inv /\ wrepeat pre inv post hist /\ event_stream (repeat body) p /\ event_stream body p)
...
let rec iodiv_repeat_proof_aux (pre : history -> Type0) (inv : trace -> Type0)
  (body : iodiv unit (winv pre inv)) (post : wpost unit) (hist : trace) (p : iopostream) (n : nat) :
  Lemma
    (requires append_stable inv /\ wrepeat pre inv post hist /\ event_stream (repeat body) p)
    (ensures (exists (m : nat). n <= m /\ inv (ipos_trace (stream_trunc p m))) /\ valid_trace hist (ipos_trace (stream_trunc p n)))
    (decreases n)
= match find_ret body [] (stream_trunc p n) with
  | Some ((), q) ->
    assert (isRet (body (find_ret_prefix body [] (stream_trunc p n)))) ;
    winv_ret pre inv body hist (find_ret_prefix body [] (stream_trunc p n)) ;
    assert (pre (hist_cons hist (ipos_trace (find_ret_prefix body [] (stream_trunc p n))))) ;
    assert (inv (ipos_trace (find_ret_prefix body [] (stream_trunc p n)))) ;
    ...
    | Tau_choice :: q' ->
      ...
      repeat_proof_aux_smaller body n p (find_ret_prefix body [] (stream_trunc p n)) q' ;
      iodiv_repeat_proof_aux pre inv body post
        (hist_cons hist (ipos_trace (find_ret_prefix body [] (stream_trunc p n))))
        (stream_drop (1 + length (find_ret_prefix body [] (stream_trunc p n))) p)
        (n - 1 - length (find_ret_prefix body [] (stream_trunc p n))) ;
      repeat_proof_aux_eqpos p (find_ret_prefix body [] (stream_trunc p n)) q' n ;
      eliminate
        exists (m : nat).
          (n - 1 - length (find_ret_prefix body [] (stream_trunc p n))) <= m /\
          inv (ipos_trace (stream_trunc (stream_drop (1 + length (find_ret_prefix body [] (stream_trunc p n))) p) m))
      returns exists (m : nat). n <= m /\ inv (ipos_trace (stream_trunc p m))
      with _. begin
        append_stable_inst
          inv
          (ipos_trace (find_ret_prefix body [] (stream_trunc p n)))
          (ipos_trace (stream_trunc (stream_drop (1 + length (find_ret_prefix body [] (stream_trunc p n))) p) m)) ;
        repeat_proof_aux_eqtrace p (find_ret_prefix body [] (stream_trunc p n)) q' n m
      end ;
  ...
  | None ->
    ...
    returns (exists (m : nat). n <= m /\ inv (ipos_trace (stream_trunc p m))) /\ valid_trace hist (ipos_trace (stream_trunc p n))
    with _. iodiv_repeat_proof_aux_inf pre inv body post hist p n
    and  _. iodiv_repeat_proof_aux_overfin pre inv body post hist p n

let iodiv_repeat_proof (pre : history -> Type0) (inv : trace -> Type0)
  ...
  Lemma
    ...
    (ensures post (Inf p') /\ valid_postream hist p')
= introduce forall (n : nat). (exists (m : nat). n <= m /\ inv (ipos_trace (stream_trunc p m))) /\ valid_trace hist (ipos_trace (stream_trunc p n))
  with begin
    iodiv_repeat_proof_aux pre inv body post hist p n
  end ;
    ...
    with _. begin
      ...
      with begin
        ...
        with _. begin
          introduce p `uptotau` p' ==> post (Inf p') /\ valid_postream hist p'
          with _. begin
            iodiv_repeat_proof pre inv body post hist p p'
          end
        end
      end
    end
