...
Section State.
  ...
  Record P A (w : W A) := {
    Ppre : Prop ;
    Phpre : ∀ post s, w post s → Ppre ;
    Pfun : D (Ppre → A) (λ post s₀, w (λ s₁ x, post s₁ (λ _, x)) s₀)
  }.
  ...
  Proof.
    ...
    eexists ?[pre].
    1: admit.
    simple refine (subcompᴰ (bindᴰ c.(Pfun) (λ x, mapᴰ _ _))).
    (* The pre of f depends on the argument so we need map when binding
      but how do we give an argument to f?
    *)
