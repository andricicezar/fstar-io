...
let postream_prepend_nil #op #s (ps : postream op s) :
...
= if n >= length p
  then begin
    move_requires (index_extensionality (postream_trunc (postream_prepend p ps) n)) (p @ postream_trunc ps (n - length p)) ;
    postream_trunc_length (postream_prepend p ps) n ;
    append_length p (postream_trunc ps (n - length p)) ;
    postream_trunc_length ps (n - length p) ;

    forall_intro (index_postream_trunc (postream_prepend p ps) n) ;
    forall_intro (index_append p (postream_trunc ps (n - length p))) ;
