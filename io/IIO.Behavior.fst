...
val beh_giio : (list string -> GTot (dm_giio int AllActions trivial_hist)) -> trace_property #event
let beh_giio ws tr =
  match tr with
  | Infinite_trace _ _ -> False
  | Finite_trace args lt res -> 
   (* We verify specs of whole programs, thus, instead of having
      properties forall histories, we can specialize it for the
      empty history *)
    forall p. dm_giio_theta (ws args) p [] ==> p lt res

val _beh : (list string -> IIO int AllActions (fun _ -> True) (fun _ _ _ -> True)) -> trace_property #event
let _beh ws =
  beh_giio (fun args -> 
    let ws' : unit -> IIO int AllActions (fun _ -> True) (fun _ _ _ -> True) = (fun () -> ws args) in
    __reify_IIOwp (fun () -> ws args))

irreducible
val beh : (list string -> IIO int AllActions (fun _ -> True) (fun _ _ _ -> True)) ^-> trace_property #event
