...
let tau_cmd cmd : bool = cmd = GetTrace
...
let extend_trace
  ...
  (argz : args cmd)
  (rez  : res cmd) : trace =
  if tau_cmd cmd then t
  else convert_call_to_event cmd argz rez :: t

(** the behavior of a program is influenced by the
  past behavior, but the GetTrace primitive does not
  give us enough information about how the trace is
  updated and how is it obtained when needed. **)
let rec behavior #a
  (m : iio a) : set_of_traces a =
  match m with
  | Return x -> fun t -> t == ([], x)
  | Call GetTrace arg fnc -> (fun (t', r') ->
    exists (h:trace). behavior (fnc h) (t', r'))
  | Call (cmd:io_cmds) arg fnc -> (fun (t', r') ->
      exists r t. (
...
let beh_shift_trace
  ...
  Lemma
    (requires
      (behavior
        (Call cmd arg fnc)
        ((extend_trace t cmd arg rez), r)))
    (ensures (behavior (fnc rez) (t, r)))
    by (ExtraTactics.blowup ();
        bump_nth 11;
        dump "h")=
  ()
...
let beh_extend_trace
  ...
  (r:'a) :
  Lemma
    (requires (behavior (fnc rez) h (t, r)))
    (ensures (
      behavior
        (Call cmd arg fnc)
        h
...
//     (ensures (
//       behavior
//         (io_bind 'a 'b (Call cmd arg fnc) k)
//         ((convert_call_to_event cmd arg rez) :: t, r))) = ()

// let beh_extend_trace_in_bind_silent
//   (cmd : tau_cmds)
//   (arg : io_args cmd)
//   (rez : io_resm cmd)
//   (fnc : (io_resm cmd) -> io 'a)
...
//     ) else (
//       let rez = pick (fun rez -> behavior (fnc rez) (t1, (Inl r1))) in
//       beh_bind_inl (fnc rez) k r1 t1 t2 r2
//       beh_extend_trace_in_bind cmd arg rez fnc k (tt1@t2) r2
//     )
//   end
