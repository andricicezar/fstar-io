...
let ex1_interface : iio_interface = {
  spec_pi = ex1_spec_pi;
  inst_pi = ex1_inst_pi;
...
val ex1_prog : prog_iio ex1_interface
let ex1_prog #fl ctx () : IIO (resexn unit) (fl + IOActions) (fun _ -> True) (fun _ _ _ -> True) =
  let _ = ctx () in
  Inl ()

val ex1_ctx : ctx_iio ex1_interface
let ex1_ctx #fl eff_rcs io_acts () : IIO (resexn file_descr) fl (fun _ -> True) (ex1_ct_post ()) = 
  io_acts Openfile "/etc/passwd"

val ex1_ctx_t : ctx_ilang (comp_int_iio_ilang ex1_interface)
let ex1_ctx_t #fl io_acts () : IIOpi (resexn file_descr) fl (comp_int_iio_ilang ex1_interface).spec_pi = 
  io_acts Openfile "/etc/passwd"
