...
let handler_env =
  extend TFDesc (extend (TArr (TPair TFDesc TBytes) (TSum TUnit TExn)) (extend (TArr TBytes (TSum TUnit TExn)) (extend TBytes empty)))
...
let wrap_handler (e:exp) (h:typing handler_env e (TSum TUnit TExn)) : tgt_handler =
 fun #fl sec_io (client : int) (req : Bytes.bytes) (send : Bytes.bytes -> MIOpi (either unit exn) fl pi _) ->
   ...
   let handler_venv = vextend #fl #mymst #pi #TFDesc client (vextend #fl #mymst #pi #(TArr (TPair TFDesc TBytes) (TSum TUnit TExn)) write (vextend #fl #mymst #pi #(TArr TBytes (TSum TUnit TExn)) send (vextend #fl #mymst #pi #TBytes req (vempty #fl #pi #mymst)))) in
