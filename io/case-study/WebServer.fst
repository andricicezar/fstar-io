...
let get_req (fd:file_descr) :
  MIO (resexn Bytes.bytes) IOActions (fun _ -> True) (fun h r lt -> exists limit r'. (Inl? r <==> Inl? r') /\ lt == [ERead Prog (fd, limit) r']) =
...
let check_send_pre : tree pck_dc = 
  Node 
...
let check_handler_post : tree pck_dc =
