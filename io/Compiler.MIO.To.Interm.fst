...
let eff_dc_typ_cont_post (dc:dc_typ 'a 'b) (h0:erased trace) (x:'a) (y:'b) (h1:trace) (b:bool) (lt:trace) : Type0 =
  (h0 `suffix_of` h1) /\
  (let the_lt = get_local_trace h0 h1 in
    ...
    /\ (b <==> dc x h0 y the_lt)) // the result is the same as the pure dc.

(* Effectful runtime check for the result, given initial value
and initial history *)
type eff_dc_typ_cont (fl:erased tflag) (t1:Type u#a) (t2:Type u#b) (dc:dc_typ t1 t2) (x:t1) (h0:erased trace) =
  y:t2 -> MIO bool fl (fun h1 -> h0 `suffix_of` h1) (eff_dc_typ_cont_post dc h0 x y)
...
type eff_dc_typ (fl:erased tflag) (#t1 #t2:Type) (dc:dc_typ t1 t2) =
  x:t1 -> MIO (h0:(erased trace) & eff_dc_typ_cont fl t1 t2 dc x h0)
             fl
             (fun _ -> True)
             (fun h1 (| h0, _ |) lt -> h1 == reveal h0 /\ lt == [])
...
let enforce_dc #argt #rett dc x =
  let h0 = get_trace () in
  let cont : eff_dc_typ_cont AllActions argt rett dc x (hide h0) =
    (fun y -> (
      let h1 = get_trace () in
...
class safe_importable (styp : Type u#a) (fl:erased tflag) (pi:policy_spec) (dcs:tree (pck_dc u#c u#d)) = {
  ...
  c_ityp : interm ityp fl pi;
  [@@@no_method]
  safe_import : ityp -> typ_eff_dcs fl dcs -> styp; 
}
...
instance interm_safe_importable_super styp fl pi dcs (d : safe_importable styp fl pi dcs) : interm d.ityp fl pi = d.c_ityp
...
let enforce_pre
  ...
  (x:t1) :
  MIO (resexn t2) fl (fun _ -> True) (trivialize_new_post (fun x h -> dc x h () []) (fun _ -> post) ()) =
  let (| h, eff_dc' |) : (h0:(erased trace) & eff_dc_typ_cont fl unit unit dc () h0) = eff_dc () in
  if eff_dc' () then f x 
...
let enforce_pre_args
  ...
  let (| h, eff_dc' |) : (h0:(erased trace) & eff_dc_typ_cont fl t1 unit dc x h0) = eff_dc x in
  if eff_dc' () then f x 
  else Inr Contract_failure
...
let rityp_eff_dc #fl #a #b #c #d #dc eff_dc (x:c) = 
    let (| h0, cont |) = eff_dc x in
    let cont' : eff_dc_typ_cont fl c d (rityp_dc dc) x h0 = (fun (y:d) -> cont y) in
    (| h0, cont' |)
