...
type c2typ (#arg:Type u#a) (#res:Type u#b) (pre:arg -> trace -> Type0) (post:arg -> trace -> resexn res -> trace -> Type0) (pi:policy_spec) (irc:irc_typ arg (resexn res)) =
  squash (forall x h lt r. pre x h /\ enforced_locally pi h lt /\ irc x h r lt ==> post x h r lt)
...
let test1_ct_irc : irc_typ unit (resexn file_descr) =
  fun () h (rfd:resexn file_descr) lt -> Inl? rfd && (is_open (Inl?.v rfd) (rev lt @ h))

let test1_ct_rc : rc_typ mst1 test1_ct_irc =
  fun () s0 rfd s1 ->
    match rfd with
    | Inl fd -> List.mem fd s1
    | Inr _ -> false

let test1_ct_rcs : tree (pck_rc mst1) = 
...
let test1_ct_importable (fl:erased tflag) : safe_importable (test1_ct fl) mst1 test1_pi test1_ct_rcs fl =
