...
let elab_typ_test5 #rrs () = 
  ...
  let h1 = get() in
  assume(self_contained_region_inv rrs h1);
  v
...
let progr2 #_ #rs #rrs #rrp ctx = (** If this test fails, it means that the spec of f does not give [automatically] separation  **)
  // 
...
val progr3: 
  ...
  #rrs:rid ->
  #rrp:rid ->
  ctx:(elab_typ (TArr (TArr TUnit (TRef TNat)) TUnit) rrs) ->
  ST unit 
    (requires (fun h0 -> 
      self_contained_region_inv rrs h0 /\
      sep rp rrp rs rrs h0 /\
...
let progr3 #_ #rs #rrs #rrp f = (** If this test fails, it means that the spec of f does not give [automatically] separation  **)
  let secret = ralloc rrs 5 in
  let cb = (fun () -> write' secret (!secret + 1); !secret % 2) in
  f ();
  assert (False);
  let h1 = get () in
  eliminate forall a (c:target_lang (reference a)) (r:reference a). frameOf r == rrs ==> 
