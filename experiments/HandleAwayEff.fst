...
let rec theta (m:free_req 'a) : pure_wp 'a =
  ...
  | Op k ->
    pure_bind_wp _ _ (as_pure_wp (fun p -> forall x. p x)) (fun x -> theta (k x))
  | Require pre k -> pure_bind_wp _ _ (as_pure_wp (fun p -> pre /\ p (() <: squash pre))) (fun r -> theta (k r))

let dm (a:Type) (wp:pure_wp a) =
  m:(free_req a){forall p. wp p ==> theta m p}

let dm_return (x:'a) : dm 'a (pure_return 'a x) = Return x

let dm_bind (#a:Type u#a) (#b:Type u#b) (#wp1:pure_wp a) (#wp2:a -> pure_wp b)
  (m:dm a wp1)
  (k:(x:a -> dm b (wp2 x))) :
  dm b (pure_bind_wp a b wp1 wp2) =
  admit ()
...
instance source_arrow (t1:Type) {| source t1 |} (t2:Type) {| source t2 |} (wp:t1 -> pure_wp t2) : source (x:t1 -> dm t2 (wp x)) = { _empty = () }

let test0 l: source (x:int -> dm int (as_pure_wp (fun p -> x > 5 /\ (forall r. p r)))) =
  source_arrow int int (fun x -> (as_pure_wp (fun p -> x > 5 /\ (forall r. p r))))

//let test : source (x:int -> dm (y:int -> dm int (as_pure_wp (fun p -> x == y /\ (forall r. p r)))) (as_pure_wp (fun p -> x > 5 /\ (forall r. p r)))) = solve
...
let rec behT (m:free 'a) : pure_wp 'a = 
  match m with
  | Ret x -> pure_return _ x
  | Call k ->
    pure_bind_wp _ _ (as_pure_wp (fun p -> forall x. p x)) (fun x -> behT (k x))
...
let rec lift_m (m:free 'a) : dm 'a (as_pure_wp (fun p -> forall r. p r)) =
  match m with
  | Ret x -> Return x
  | Call k -> Op (fun x -> lift_m (k x))

let rec handle_m (m:dm 'a (as_pure_wp (fun p -> forall r. p r))) : free 'a =
  match m with
  | Return x -> Ret x
  | Require pre k -> begin
...
instance liftable_arrow (t1 t2:Type) {| c1:handleable t1 |} {| c2:liftable t2 |} : liftable (t1 -> free t2) = {
  ...
  s = c1.s -> dm c2.s (as_pure_wp (fun p -> forall r. p r));
  s_tc = source_arrow c1.s #c1.s_tc c2.s #c2.s_tc (fun x -> as_pure_wp (fun p -> forall r. p r));
...
let sc (ps:(int -> dm int (as_pure_wp (fun p -> forall r. p r))) -> dm int (as_pure_wp (fun p -> forall r. p r))) (ct:int -> free int) : Type0 =
...
let sc' (ct:(int -> free int) -> free int) (ps:int -> dm int (as_pure_wp (fun p -> forall r. p r))) : Type0 =
  behS (dm_bind (lift_m (ct ((handleable_arrow int int).handle ps))) (fun r -> dm_return (lift r))) `eq_beh (==)` behT (ct (handle ps))
