...
let rec theta #a #cmd1 #cmd2 (#event:Type)
  ...
  (m:free cmd1 cmd2 a) : Tot (hist #event a) (decreases m) =
  match m with
  | Return x -> hist_return x
  | Call1 c op k ->
      hist_bind (cwp1 c op) (fun ri -> theta cwp1 cwp2 (k ri))
  | Call2 c op k ->
      hist_bind (cwp2 c op) (fun ri -> theta cwp1 cwp2 (k ri))
...
let rec lemma_theta_is_lax_morphism_bind #a #b #cmd1 #cmd2 (#event:Type)
  ...
  | Call1 c op k ->
    calc (hist_ord) {
      hist_bind (theta cwp1 cwp2 m) (fun x -> theta cwp1 cwp2 (f x));
      == {
        assert (hist_bind (theta cwp1 cwp2 (Call1 c op k)) (fun x -> theta cwp1 cwp2 (f x))
           == hist_bind (theta cwp1 cwp2 m) (fun x -> theta cwp1 cwp2 (f x))) by (rewrite_eqs_from_context ())
      }
      hist_bind (theta cwp1 cwp2 (Call1 c op k)) (fun x -> theta cwp1 cwp2 (f x));
      == { _ by (compute ()) } // unfold theta
      hist_bind (hist_bind (cwp1 c op) (fun ri -> theta cwp1 cwp2 (k ri))) (fun x -> theta cwp1 cwp2 (f x));
      `hist_equiv` { lemma_hist_bind_associativity (cwp1 c op) (fun ri -> theta cwp1 cwp2 (k ri)) (fun x -> theta cwp1 cwp2 (f x)) }
      hist_bind (cwp1 c op) (fun ri -> hist_bind (theta cwp1 cwp2 (k ri)) (fun x -> theta cwp1 cwp2 (f x)));
      `hist_ord` {
        ...
        another_lemma' (cwp1 c op) rhs1 rhs2;
        assert (hist_bind (cwp1 c op) rhs1 `hist_ord` hist_bind (cwp1 c op) rhs2) by (assumption ())
      }
      hist_bind (cwp1 c op) (fun ri -> theta cwp1 cwp2 (free_bind (k ri) f));
      == { _ by (compute ()) } // unfold theta
      theta cwp1 cwp2 (Call1 c op (fun ri -> free_bind (k ri) f));
      `hist_ord` { _ by (compute ()) } // unfold free_bind
      theta cwp1 cwp2 (free_bind (Call1 c op k) f);
      == {}
      theta cwp1 cwp2 (free_bind m f);
    }
  | Call2 c op k ->
    calc (hist_ord) {
      hist_bind (theta cwp1 cwp2 m) (fun x -> theta cwp1 cwp2 (f x));
      == {
        assert (hist_bind (theta cwp1 cwp2 (Call2 c op k)) (fun x -> theta cwp1 cwp2 (f x))
           == hist_bind (theta cwp1 cwp2 m) (fun x -> theta cwp1 cwp2 (f x))) by (rewrite_eqs_from_context ())
      }
      hist_bind (theta cwp1 cwp2 (Call2 c op k)) (fun x -> theta cwp1 cwp2 (f x));
      == { _ by (compute ()) } // unfold theta
      hist_bind (hist_bind (cwp2 c op) (fun ri -> theta cwp1 cwp2 (k ri))) (fun x -> theta cwp1 cwp2 (f x));
      `hist_equiv` { lemma_hist_bind_associativity (cwp2 c op) (fun ri -> theta cwp1 cwp2 (k ri)) (fun x -> theta cwp1 cwp2 (f x)) }
      hist_bind (cwp2 c op) (fun ri -> hist_bind (theta cwp1 cwp2 (k ri)) (fun x -> theta cwp1 cwp2 (f x)));
      `hist_ord` {
        let rhs1 = fun ri -> hist_bind (theta cwp1 cwp2 (k ri)) (fun x -> theta cwp1 cwp2 (f x)) in
        let rhs2 = fun ri -> theta cwp1 cwp2 (free_bind (k ri) f) in
        introduce forall ri. (rhs1 ri) `hist_ord` (rhs2 ri) with begin
        ...
        another_lemma' (cwp2 c op) rhs1 rhs2;
        assert (hist_bind (cwp2 c op) rhs1 `hist_ord` hist_bind (cwp2 c op) rhs2) by (assumption ())
      }
      hist_bind (cwp2 c op) (fun ri -> theta cwp1 cwp2 (free_bind (k ri) f));
      == { _ by (compute ()) } // unfold theta
      theta cwp1 cwp2 (Call2 c op (fun ri -> free_bind (k ri) f));
      `hist_ord` { _ by (compute ()) } // unfold free_bind
      theta cwp1 cwp2 (free_bind (Call2 c op k) f);
      == {}
      theta cwp1 cwp2 (free_bind m f);
    }
...
let dm_cmd1 #cmd1 #cmd2 (#event:Type) (cwp1:cmd_wp cmd1 event) (cwp2:cmd_wp cmd2 event) (c:caller) #r (op:cmd1 r) :
  dm cmd1 cmd2 event cwp1 cwp2 r (hist_bind (cwp1 c op) (fun ri -> hist_return ri)) =
  Call1 c op Return

let dm_cmd2 #cmd1 #cmd2 (#event:Type) (cwp1:cmd_wp cmd1 event) (cwp2:cmd_wp cmd2 event) (c:caller) #r (op:cmd2 r) :
  dm cmd1 cmd2 event cwp1 cwp2 r (hist_bind (cwp2 c op) (fun ri -> hist_return ri)) =
  Call2 c op Return
