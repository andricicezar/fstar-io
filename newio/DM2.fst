...
let dm (a:Type) (wp:hist a) (q:pure_post a) = 
  (** ATTENTION, the LHS of hist_ord does not have the same type as the RHS. **)
  m:(io (v:a{q v})){wp `hist_ord` theta m}

unfold
let return_post (#a:Type) (x:a) : pure_post a = fun r -> r == x

let dm_return (a : Type) (x : a) : dm a (hist_return #a x) (return_post x) =
  io_return x
...
let dm_subcomp
  ...
  (_:squash (wp1 `hist_ord #'a` wp2)) :
  dm 'a wp2 q2= 
  admit ();
  m

let lemma_hist_monotonic
  (a b : Type)
  (wp_v : hist a)
  (wp_f:  a -> hist b)
  (post_v: pure_post a)
  ...
  (v : dm a wp_v post_v)
  (f : (x:a -> dm b (wp_f x) (bind_post post_v post_f))) :
  Lemma (
  	hist_bind wp_v wp_f `hist_ord` hist_bind (theta v) (fun x -> theta (f x)))
  =
  assert (wp_v `hist_ord #(r:a{post_v r})` theta v);
  ()
...
let dm_bind0
  ...
  lemma_hist_monotonic a b wp_v wp_f post_v post_f v f;
  io_bind v f
...
let l_return (a:Type) (x:a) : l_repr a True (return_post x) (hist_return x) = fun () -> dm_return a x
