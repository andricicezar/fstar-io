...
let compile_pprog1 #i ps =
 ...
 //   (safe_exportable_arrow i.ct int #i.c_ct (fun _ -> lr_post _ pre' (fun _ _ _ -> True)) ()).export ps
...
(* Note: previously this was discharged with a `compute ()` tactic, which
   no longer goes through. The proof goes through the syntactic equality of
   the whole programs instead, as in sciostar. Like there, the universe
   instantiation of comp1 must be fixed: without the annotations, the
   universes of the statement and of the proof are generalized separately,
