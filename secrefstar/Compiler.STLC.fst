...
(* Note: previously this was discharged with a `compute ()` tactic, which no
   longer goes through; the proof goes through the syntactic equality of the
   whole programs instead, as in sciostar (with the universe instantiation of
   comp1 fixed). *)
