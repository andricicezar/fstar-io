...
type step : closed_exp -> closed_exp -> (h:history) -> option (event_h h) -> Type =
  ...
  | SRead :
    r:resexn bool ->
    h:history ->
    step ERead (get_resexn_bool r) h (Some (EvRead () r))
  | SWrite :
    #arg:closed_exp ->
    #arg':closed_exp ->
    #h:history ->
    ...
    step (EWrite arg) (EWrite arg') h oev
  | SWriteReturn :
  ...
  = match st with
    ...
    | STrans #e #f2 #e' #h #_ #lt23 step_eapp step_eapp_steps -> begin
      ...
      | AppRight e1 #e2 #e2' #h #oev2 step_e2 -> begin
        ...
        sem_expr_shape_history_independence e1 h (TArr t1 t2);
        ...
        introduce indexed_irred e1 h ==> (lt1 == [] /\ e1 == ELam e11) with _. begin
