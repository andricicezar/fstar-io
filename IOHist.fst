...
val include_in : events_trace -> (set_of_traces) -> Type0
let include_in t set = set t

val subset : set_of_traces -> set_of_traces -> Type0
let subset s1 s2 = forall t. s1 t ==>  s2 t
...
let _ = assert (include_in [EOpenfile "asdf" (Inl 7)] (behavior (Cont (Call Openfile "asdf" (fun x -> Return x))))) by (
  unfold_def (`behavior);
...
exception OneExc
let _ = assert (include_in [EOpenfile "asdf" (Inr OneExc)] (behavior code1)) by (
  unfold_def (`code1);
  unfold_def (`behavior);
  unfold_def (`include_in);
  unfold_def (`convert_call_to_event);
