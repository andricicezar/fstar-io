...
let return_set (#a:Type) (x:a) : set a 1 = fun st0 y -> make_el st0 x == y

let add_set (#a:Type) (#n:nat) (s0:set a n) (x:a) : set a (n+1) =
  fun (st0:nat{st0 >= n+1}) y -> make_el st0 x == y \/ y ∈ s0 (st0-1)
let (⊕) #a #n = add_set #a #n
...
let return_poset (el:'e) : poset 'e 1 =
  ...
  (| s, empty_partial_order s, Some (fun st0 -> make_el st0 el), Some (fun st0 -> El st0 el) |)
...
let __test_poset : poset int 3 =
  (| __test_set, __test_po, Some (fun (st0:nat{st0 >= 3}) -> make_el (st0-2) 1), Some (fun id -> El id 3) |)
...
let __test_poset3 : poset int 4 =
  (| __test_set3, __test_po3, Some (fun (st0:nat{st0 >= 4}) -> make_el (st0-3) 1), Some (fun id -> El id 4) |)
...
let op (e:'e) (th_id:nat) : poset (event_typ 'e) 1 =
  ...
  (| s, empty_partial_order s, Some (fun st0 -> make_el st0 (Some (th_id, e))), Some (fun st0 -> El st0 (Some (th_id, e))) |)


(**
     bot st0                               make_el (st0-1) None        (new bot) 
      |             async                   /        \
      |          ---------->               /          \
      |                                   v           v
      v                              bot (st0-2)   make_el st0 None    (new top)
     top_r st0                            |
                                          |
                                          v
                                    top_r (st0-2)
...
let async (#n:nat) (p:poset (event_typ 'e) n) : poset (event_typ 'e) (n+2) = 
  ...
  assert (forall st0. make_el st0 None ∈ s' st0 /\ El (st0-1) None ∈ s' st0);
  let po : partial_order s' = (fun st0 x y -> (x == make_el (st0-1) None /\ y ∈ s' st0) \/
                                            (x == make_el st0 None /\ y == El st0 None) \/
                                            (x ∈ s (st0-2) /\ y ∈ s (st0-2) /\ x `po (st0-2)` y)) in
  (| s', po, Some (fun (st0:nat{st0 >= n+2}) -> make_el (st0-1) None), Some (fun st0 -> El st0 None) |)
...
let await (#e:Type) (th_id:nat) : poset (event_typ e) 1 =
  ...
  let po : partial_order s = (fun st0 x y -> (x ∈ s st0 /\ y ∈ s st0) \/ (y == make_el st0 None /\ Some? (v x) /\ fst (Some?.v (v x)) == th_id)) in
  (| s, po, Some (fun st0 -> make_el st0 None), Some (fun st0 -> El st0 None) |)
