...
let hist_bind0 (#a:Type u#a) (#b:Type u#b) (w : hist 'e a) (kw : a -> hist 'e b) : hist0 'e b =
  fun p ths h -> w (hist_post_bind ths h kw p) (univ_change ths) h
...
let hist_ord wp1 wp2 = forall ths h p. wp1 p ths h ==> wp2 p ths h

val hist_equiv : hist 'e 'a -> hist 'e 'a -> Type0
let hist_equiv wp1 wp2 = forall h thrs p. wp1 p thrs h <==> wp2 p thrs h
...
| Wait : nat -> cont:(unit -> free a e) -> free a e
...
let hist_fork0 (#e:Type u#0) (#a:Type u#a) (hist_th:hist e (Universe.raise_t u#0 u#a unit)) : hist0 e (Universe.raise_t u#0 u#a unit) =
  fun p -> hist_th (fun r -> p (Node (Cv (Universe.raise_val ()) []) [r]))
...
let rec theta m =
  ...
  | Wait id k -> let wp = (fun p thrds h ->
    ...
    (forall lt. lt `memP` as_traces rt ==>
      theta (k ()) (fun (Node (Cv x lt') rts) -> p (Node (Cv x lt') rts)) thrds (** - rt **) (List.rev lt @ h))) in
    assume (hist_wp_monotonic wp);
    wp
...
let prog4 = Fork (fun () -> Print 0 (fun () -> Return (Universe.raise_val ()))) 
                 (fun () -> Print 5 (fun () -> Wait 0 (fun () -> Print 6 (fun () -> Return ()))))
