...
instance exportable_arrow
  ...
  mk_exportable (d1.itype -> ML d2.etype)
    (fun (f:(t1 -> t2)) ->
      (fun (x:d1.itype) ->
        export (f (exn_import x)) <: ML d2.etype))

instance exportable_mlarrow
  t1 t2 {| d1:importable t1 |} {| d2:exportable t2 |} :
  Tot (exportable (t1 -> ML t2)) =
  mk_exportable (d1.itype -> ML d2.etype)
    (fun (f:(t1 -> ML t2)) ->
      (fun (x:d1.itype) -> export (f (exn_import x)) <: ML d2.etype))
...
let rec export_iio #t1 (t:iio t1) : ML t1 =
  match t with
  | Return (Inl r) -> r
  | Return (Inr r) -> FStar.All.raise r
  | Call cmd argz fnc ->
      export_iio #t1 (fnc (ml_io_execute cmd argz))
...
let _export_MIIO_0
  ...
  (x:t1) :
  ML t2 =
   let z : iio t2 = reify (f x) [] (fun _ _ -> True) in
   export_iio z
...
let _export_MIIO
  ...
  ML d2.etype =
  match import x with
  | Some x -> export (_export_MIIO_0 f x)
